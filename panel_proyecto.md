...
- **Extendido a los 9 juegos del catálogo** (sesión siguiente): todos tienen ya sus 3 trofeos difíciles.
  - **Completos y en castellano confirmado por el autor:** Dark Souls Remastered (*Rezo de una dama* 29,56 % · *Honor del Caballero* 29,59 % · *Arma de Fuego* 30,80 %) y Dark Souls II (*Maestro de los milagros* 4,6 % · *Maestro de la piromancia* · *Maestro de la magia*, estos dos últimos sin % exacto confirmado — ver más abajo).
  - **Con porcentaje e icono reales, pero nombre aún en inglés** (pendiente de que el autor confirme el nombre en castellano desde su cuenta, para no repetir el problema de las traducciones fan contradictorias): Dark Souls III, Elden Ring, Lords of the Fallen, Bloodborne, Sekiro, Astro's Playroom. Cada entrada de `juegos.json` lo marca explícitamente en su `descripcion` ("nombre pendiente de confirmar en castellano").
  - **Demon's Souls:** nombres confirmados por el autor (*Trofeo del Sabio*, *Rey de los Anillos*, *Trofeo de la Santa*) y porcentaje real (27,70 % / 27,97 % / 28,72 %, vía Exophase), pero **sin icono**: no se ha encontrado la página de este juego en psn100.net (posible mezcla con la versión antigua de PS3 en otras webs de trofeos).
  - **Dark Souls II, dos de sus tres trofeos sin porcentaje exacto:** solo "Maestro de los milagros" tiene un dato sólido (4,6 %, Guinness World Records); "piromancia" y "magia" se confirmaron por nombre pero no por cifra — ninguna fuente consultada la publicaba. La ficha ya no muestra "0 %" falso en este caso: `juego.html` se actualizó para mostrar "Porcentaje exacto pendiente de confirmar" cuando `porcentaje` es `null`, y oculta el hueco de icono cuando `icono` es `null` (en vez de un icono roto).
...
- **`porcentaje: null` e `icono: null` están soportados en `trofeos_dificiles`** (ver Dark Souls II y Demon's Souls) para cuando no se encuentra el dato real: `juego.html` muestra "Porcentaje exacto pendiente de confirmar" en vez de un falso "0 %", y oculta el hueco en vez de un icono roto. Mejor dejarlo pendiente y visible que rellenarlo con un dato inventado.
