...
*Última actualización: sesión 4 — añadidas 7 fichas nuevas al catálogo (2026-08-22), todavía sin subir a producción*
...
- [x] Servir imágenes/carátulas desde el propio dominio. ✅ Endpoint `/api/caratula/:appid` proxea desde Steam con caché de 1 día → **resuelto el bloqueo de Brave/iOS**. ⚠️ **Aprendizaje (al añadir Resident Evil Requiem):** los juegos de Steam más nuevos ya no usan la ruta predecible `steamcdn-a.akamaihd.net/steam/apps/{appid}/...` (Valve movió esos assets a URLs con hash). Añadido un último recurso: si las rutas predecibles fallan, el endpoint consulta `store.steampowered.com/api/appdetails` y usa el `header_image` real. Funciona, pero ese `header_image` es un banner apaisado (no la portada vertical 600x900), así que en juegos muy nuevos la carátula puede verse algo recortada en las tarjetas. Se le avisó al autor; **pendiente de decidir** si se deja así o se sube una carátula vertical propia para Resident Evil Requiem.
...
- [ ] Empezar a **rellenar fichas** de los juegos favoritos del autor. 🟡 En marcha: **22 juegos** en el catálogo (`juegos.json`). ⚠️ **Esta ronda de 7 juegos está en el árbol de trabajo local, sin `git add`/commit ni push todavía** — pendiente de que el autor la revise y decida subirla.
  - **Ronda de 7 juegos nuevos (sesión 4, 2026-08-22) — pendiente de revisión del autor.** A diferencia de las rondas anteriores, en esta el autor pidió explícitamente sacar dificultad/duración/tedio de fuentes abiertas (PowerPyx, psn100.net, PSNProfiles cuando no bloqueaba el acceso) en vez de jugarlos él primero, con la idea de revisarlo después. Cada ficha con datos así lleva la marca **`[PENDIENTE DE REVISIÓN DEL AUTOR]`** al principio de `tedio_comentario` en `juegos.json`, para distinguirla a simple vista de una opinión real de Alberto.
    - **Lies of P** (Steam `1627720`, también PS4/PS5, Xbox y Switch 2): única ficha de esta ronda con datos **reales del autor** — está "casi platinado". `offline`, tedio 3,5/5 ("ni difícil ni tedioso, la dificultad está en el propio juego"); se saca en dos vueltas, dejando para la New Game+ el trofeo de escuchar todos los discos de vinilo. Dificultad de consenso 6/10, ~50h.
    - **Horizon Zero Dawn Remastered** (Steam `2561580`, también PS5 — la Remastered no salió para PS4): `offline`, sin perdibles ni trofeos de dificultad. Dificultad de consenso 3/10, ~60-80h; el tedio viene del puro completismo del mapa y de las pruebas "Sol Radiante" de las Zonas de Caza.
    - **Indiana Jones and the Great Circle** (Steam `2677660`, también PS5 y Xbox Series X|S): `offline`, sin multijugador ni perdibles, todo en una sola partida con viaje rápido. Dificultad de consenso 2/10, ~30-50h; el tedio viene de coleccionables masivos, sobre todo las 497 notas de diario del trofeo "Archivist".
    - **Mortal Shell** (Steam `1110910`, también PS4/PS5, Xbox y Switch): `offline`. Dificultad de consenso 6/10, ~20-40h; varios trofeos exigen partidas concretas planificadas (terminar sin vincularse a ninguna carcasa, terminar en forma obsidiana).
    - **Mortal Shell II** (Steam `2584270`, también PS5 y Xbox Series X|S): ⚠️ el autor lo había descrito como "en desarrollo", pero **ya salió el 20/08/2026** (2 días antes de esta sesión) — se le avisó y decidió meterlo igualmente con ficha básica, sin haberlo jugado todavía. `offline`. Dificultad de consenso 6/10, ~40-60h, con 2 de sus 3 trofeos perdibles ya en el tramo inicial del juego.
    - **Crimson Desert** (Steam `3321460`, también PS5 y Xbox Series X|S): el autor no lo tiene; ⚠️ también **ya salió, el 19/03/2026** (no estaba pendiente de lanzamiento como él pensaba). `offline`, sin perdibles, pero **200-300+ horas** de consenso para el platino — un platino larguísimo por volumen, no por dificultad puntual.
    - **Silent Hill 2 Remake** (Steam `2124490`, también PS5 y Xbox Series X|S): el autor lo intentó hace tiempo y no lo terminó, por los muchos finales. `offline`. Sin selección de capítulo: todo lo no automático es perdible dentro de la misma partida. Hacen falta 6 de los 8 finales (se pueden conseguir varios recargando un guardado manual antes de la decisión final, sin rejugar entero) más pruebas de reto en NG+ (menos de 10h, sin radio, sin armas de fuego). Dificultad de consenso 5/10, ~20-25h.
    - **Iconos de trofeos difíciles:** verificados con `curl` (responden 200) para Lies of P, Horizon Zero Dawn y Mortal Shell, sacados de psn100.net. Para Indiana Jones, Mortal Shell II, Crimson Desert y Silent Hill 2 no se encontró su página en psn100.net (parece que aún no los tiene indexados, o el buscador del sitio no los devuelve) — sus `trofeos_dificiles` llevan `icono: null`, igual que ya pasaba con Dark Souls II.
    - **Trofeos difíciles, excluidos los de DLC de pago:** en Lies of P y Mortal Shell los 3 trofeos con menor % de consecución pertenecían a un DLC de pago (Overture / The Virtuous Cycle) y no al platino base — se descartaron y se repitió la búsqueda solo con trofeos del juego base, mismo criterio que ya se usó con Resident Evil 4 y su DLC "Separate Ways".
    - Ninguno de los 7 marcado `destacado` en portada.
  - **Ghost of Tsushima** (Steam `2215430`, también en PS4/PS5): con Steam, `offline` (Legends es multijugador aparte, con su propia lista de trofeos, no hace falta para este platino), sin destacar, sin guía todavía. Dificultad de consenso 3/10, duración ~30-40h, **cero perdibles** (platino de pura exploración/completismo, sin exigencias de dificultad). Tedio del autor 2,5/5 — lo sacó hace tiempo y no recuerda mucho, pero encaja con el perfil de "sin prisa, sin exigencia" que confirma la investigación.
  - **Resident Evil Requiem** (Steam `3764200`, también en PS4/PS5/Xbox, publicado feb. 2026): con Steam, `offline`, sin destacar, sin guía todavía. Dificultad de consenso 4/10, duración ~20h, casi todo perdible salvo 9 trofeos de historia. Tedio del autor 3,5/5 — lo sacó hace poco, tuvo que completarlo varias veces pero no es el más duro de la saga (con paciencia y comprando munición/armas infinitas tras la primera vuelta, la dificultad más alta Insanity se vuelve manejable). Mezcla de su experiencia + PowerPyx/psn100.net.
  - **Resident Evil 2 Remake** (Steam `883710`), **Resident Evil 3 Remake** (Steam `952060`) y **Resident Evil 4 Remake** (Steam `2050650`): con Steam (carátula y jugadores desde el proxy), `offline`, ninguno destacado, sin vídeo guía todavía (decisión explícita del autor, "por ahora"). Dificultad/duración de PowerPyx: RE2 6/10 · ~30h · RE3 4/10 · ~25h (el más corto) · RE4 6/10 · ~30-40h. Tedio del autor: RE2 4/5 (exige rango S+ en Hardcore contrarreloj y con guardados limitados — verificado el límite exacto: 2h30 Leon / 2h Claire, máx. 3 guardados), RE3 3/5 (parecido pero más corto, y las mejoras comprables en la tienda lo suavizan), RE4 4/5 (exige rejugar para desbloquear armas infinitas; lo tedioso son los tesoros coleccionables, no hay muchos perdibles). RE3 incluye Resident Evil Resistance (multijugador) pero no aporta trofeos a este platino. RE4 tiene el DLC de pago "Separate Ways" con su propia lista de trofeos aparte, no hace falta para este platino.
  ...
  - **Demon's Souls** (`demons-souls`, remake PS5 2020): sin Steam, `con_amigos` (los 2 trofeos multijugador — fantasma azul/negro — son obligatorios pero se hacen con un amigo por contraseña, no dependen de población online), tedio 4,5/5 (objetos, anillos y hechizos/milagros perdibles ligados a la tendencia del mundo/personaje — dato de PowerPyx/PSNProfiles verificado antes de escribir el comentario), dificultad 7/10 y duración ~45-50 h de consenso. Sin destacar. Carátula ✅ (`public/img/demons_souls.png`; renombrado, tenía una errata "deamons").
  - **Star Wars Outlaws** (appid Steam `2842040`, también en PS5/Xbox): con Steam (carátula y jugadores en vivo desde el proxy, sin archivo propio), `offline` (platino 100% sin online, verificado en PowerPyx y PSNProfiles), tedio 3,5/5 del autor ("ameno por el mundo abierto, pero pesado buscar materiales de mejora y coleccionables para los aspectos"), dificultad de consenso 3/10, duración ~30-40 h. **Destacado en portada** (a petición del autor, sustituyendo a Dark Souls Remastered). Sin vídeo guía todavía (decisión explícita del autor, "por ahora"). Trofeo "Good Listener" tiene una ventana concreta en una misión secundaria pero no es perdible de forma permanente (se puede rehacer en mundo abierto tras los créditos) — hay reportes de un bug donde el NPC no aparece.
...
- [x] Lista de **"próximos platinos"** (deseados) en el perfil. ✅ Tabla nueva `deseados` en Supabase (RLS: lectura pública, escritura/borrado solo del dueño). Sección propia en `perfil.html` bajo "Mis platinos", con botón "+ Añadir a la lista" que reutiliza el modal de búsqueda existente (ahora con dos modos). Cada juego muestra su estado (vivo/moribundo/muerto, vía nuevo endpoint `/api/deseados?ids=...`) y el tedio del autor, **ordenados por urgencia** (muerto/moribundo primero) para ayudar a priorizar. Limpieza automática: al platinar un juego (a mano o por sincronización PSN) se quita solo de la lista de deseados. Probado en local (endpoint, RLS con Supabase real, y flujo completo en el navegador confirmado por el autor).
...
- [x] **Lista de "próximos platinos" en el perfil.** ✅ Ver detalle arriba (sección "Diseño"/tabla de estado). *(Duplicado de la tarea de arriba — misma tarea, ya hecha.)*
...
Muestra los tres trofeos más difíciles de cada juego (selección del autor entre los menos conseguidos por la comunidad), con su icono y una breve descripción. **No se muestra porcentaje.**
- **Cambio de enfoque respecto a la idea original:** en vez de conectar `psn-api`/`PSN Leaderboard` en vivo (más complejo: hace falta `npCommunicationId` por juego, decidir caché, etc.), se rellena **a mano por juego**, igual que el resto de datos del catálogo (tedio, dificultad, planificación). Campo `trofeos_dificiles` en `juegos.json` (array de `{nombre, descripcion, icono}`), servido automáticamente por `/api/juego/:appid` y pintado en una sección de `public/juego.html`.
- **Sin porcentaje, a propósito:** se intentó verificar el % exacto de consecución de cada trofeo (psn100.net, Exophase…), y se consiguió en 7 de los 9 juegos, pero no en los 3 de Dark Souls II. Decisión del autor: mejor que **ningún** juego muestre porcentaje (por consistencia) a que unos lo tengan y otros no — se quitó de los 9 juegos (27 trofeos). La ficha ahora usa el espacio de descripción para explicar qué hay que hacer, no un dato numérico.
- **Fuente de los nombres/iconos:** PlayStation, sin extrapolar de Steam. Se excluye siempre el propio trofeo de platino de la lista (por definición es el más raro, no aporta información). Iconos enlazados desde su origen (verificado que la URL responde con la imagen real antes de usarla) — práctica habitual del sector, igual que hace PSNProfiles.
- **Estado por juego (los 9 completos):**
  - **Nombre en castellano confirmado por el autor:** Dark Souls Remastered (*Rezo de una dama* · *Honor del Caballero* · *Arma de Fuego*), Dark Souls II (*Maestro de los milagros* · *Maestro de la piromancia* · *Maestro de la magia*), Demon's Souls (*Trofeo del Sabio* · *Rey de los Anillos* · *Trofeo de la Santa* — icono encontrado en psntrophyleaders.com, tras no localizar la página de este juego en psn100.net).
  - **Nombre aún en inglés, pendiente de que el autor lo confirme desde su cuenta:** Dark Souls III, Elden Ring, Lords of the Fallen, Bloodborne, Sekiro, Astro's Playroom, **Star Wars Outlaws**, **Resident Evil 2 Remake, Resident Evil 3 Remake, Resident Evil 4 Remake, Ghost of Tsushima, Resident Evil Requiem**, **Lies of P, Horizon Zero Dawn Remastered, Indiana Jones and the Great Circle, Mortal Shell, Mortal Shell II, Crimson Desert, Silent Hill 2 Remake** (nombres sacados de guías en inglés). (El seguimiento de este pendiente vive solo aquí, en el panel — ya no se anota dentro de `juegos.json`, para no filtrar notas internas a los visitantes de la web.)
  - **Aprendizaje de esta ronda:** en un primer intento se rellenaron varios nombres de trofeos con **traducciones al castellano inventadas por el asistente** (sin verificar) — se detectó y se corrigió a tiempo, volviendo al nombre en inglés hasta tener confirmación real. Ver nota en "Notas y riesgos conocidos" más abajo.
...
- **Completado en 9 de los 22 juegos del catálogo.** Dark Souls Remastered, Dark Souls II, Sekiro, Bloodborne, Elden Ring, Lords of the Fallen y Demon's Souls con un vídeo de **GameInTheHole** cada uno; Dark Souls III con **dos** vídeos (GameInTheHole + **Acre**); Astro's Playroom con un vídeo de **GPAP360**. Confirmado en producción (`platinoradar.com`). Faltan **Star Wars Outlaws, Resident Evil 2 Remake, Resident Evil 3 Remake, Resident Evil 4 Remake, Ghost of Tsushima, Resident Evil Requiem** (decisión explícita del autor de dejarlos sin guía por ahora) y los **7 juegos de la ronda de la sesión 4** (Lies of P, Horizon Zero Dawn Remastered, Indiana Jones and the Great Circle, Mortal Shell, Mortal Shell II, Crimson Desert, Silent Hill 2 Remake), que tampoco tienen `guias_video` todavía.
...
- ⚠️ **Nunca inventar una traducción al castellano de un nombre de trofeo, ni siquiera como "mejor estimación".** En la ronda de rellenar los 8 juegos restantes, el asistente escribió por su cuenta nombres en castellano para varios trofeos de Dark Souls III, Elden Ring y Lords of the Fallen sin que el autor los confirmara — justo el mismo problema (traducción no oficial) que motivó la regla anterior. Se detectó a tiempo y se revirtió a inglés. **Regla concreta:** si el autor no ha confirmado el nombre en castellano de un trofeo, se deja en inglés (el nombre real que da Sony) y se marca explícitamente como pendiente en el campo `descripcion`, nunca se rellena con una traducción propia.
- **`trofeos_dificiles` no lleva porcentaje** (decisión del autor: si no se puede verificar en todos los juegos, mejor no mostrarlo en ninguno) y su campo `icono` puede ser `null` (juego.html oculta el hueco en vez de un icono roto). Las notas internas de seguimiento (p. ej. "nombre pendiente de confirmar") **no van dentro de `juegos.json`** porque ese archivo se sirve tal cual a través de la API pública — cualquier nota ahí la vería un visitante de la web. El seguimiento de pendientes va solo en este panel.
